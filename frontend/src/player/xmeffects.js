...
export function XMEffects(player) {
    console.log('XMEffects:: Mixing in effects');
    ...
    function eff_t1_h(ch) {
        ...
        if (player.xm.global_volumeslide !== undefined) {
            player.xm.global_volume = Math.max(
                0,
                Math.min(player.max_global_volume, player.xm.global_volume + player.xm.global_volumeslide)
            );
        }
    }
